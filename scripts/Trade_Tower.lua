...
local function removeAllFromMarketPlace()
    for _, frame in pairs(LocalPlayer.PlayerGui.Gui.Frames.Market.Body.Holder:GetChildren()) do
        if frame:IsA("Frame") and frame.Name == LocalPlayer.Name then
            local itemName = frame.InfoBox:WaitForChild("ItemName").Text
            spawn(function()
                local args = {
                    [1] = "Marketplace",
                    [2] = itemName
...
--[[
local Countdown = game:GetService("Players").wasntlooking.PlayerGui.Gui.Frames.Jackpot.GameList.Holder.ServerHosted
local jackpotLPInfo = {
    [1] = {
        ["total"] = 0,
        ["itemsAlreadyIn"] = 0
...
local auto = window:Tab("Auto Farm")
local jp = window:Tab("Auto Jackpot [DISCONTINUED FOR NOW]")
local acf = window:Tab("Auto Coinflip")
local ac = window:Tab("Auto Crash")
local ats = window:Tab("Auto Tix Stock")
local atd = window:Tab("Auto Tix Duels")
local mptab = window:Tab("Auto Marketplace")
local target = window:Tab('Target')
...
mpsec = mptab:Section('Auto Marketplace')
...
miscsec:Button("Save Settings",function()
    save()
end)

mpsec:Toggle("Auto Marketplace",false,"Toggle",function(v)
    getgenv().automarketplace = v
    sellCurrentItems()
end)

mpsec:Slider("Min. Value",1000,1000000,1,1000,"Slider",function(v)
    maxAutomarketSell = v
    sellCurrentItems()
end)

mpsec:Slider("Max. Value",1000000,5000000,10000,10000,"Slider",function(v)
    maxAutomarketSell = v
    sellCurrentItems()
end)

mpsec:Slider("% of value to list",104,500,10,1,"Slider",function(v)
    percentToMarketSell = v/100
end)

mpsec:Button("Remove all items from marketplace",function()
...
autosec4:Toggle("Auto collect supply drops (disabled)",Settings.autocollectsupplydrops,"Toggle",function(v)
...
FLYSPEED = 20
miscsec:Button('Fly (X)',function(v)
...
miscsec:Button('Disable AFK',function()
...
miscsec:Button('Free Korblox',function(v)
    local Leg = 'Right' 
    local plr = game.Players.LocalPlayer
...
if isPremium[game.Players.LocalPlayer.UserId] then
    ZenLib:Notification('PREMIUM',"Thanks for being a premium user! Mod perks and features loaded.")
    ...
    premium:Toggle("No premium features yet",false,"Toggle",function()
    end)
